...
const CustAccForm = () => {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      const response = await axios.patch(
        `http://localhost:8000/api/customerInfoUpdate/${email_check}/`,
        requestData
      );
      ...
      console.error(
        "Error during updating customer account info:",
        error.response.data
      );
  ...
  return (
    <div className="update-form-container">
      ...
      <button className="update-form-button" onClick={handleRegister}>
        Update Info
      </button>
    </div>
  );
